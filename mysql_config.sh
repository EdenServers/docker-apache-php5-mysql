...
if [ ! -f /mysql_configured ]; then
    ...
    mysqladmin create edenservers

    echo "=> Creating MySQL WP user and granting external access";
    mysql -uroot -e "CREATE USER 'edenservers'@'%' IDENTIFIED BY 'edenservers';"
    mysql -uroot -e "GRANT ALL PRIVILEGES ON *.* TO 'edenservers'@'%' WITH GRANT OPTION;"
